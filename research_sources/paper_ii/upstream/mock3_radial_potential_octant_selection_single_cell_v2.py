# Radial Mock-3: potential-flow reconstruction with direction-dependent selection
# Corrected v2: legacy geometry-cache compatibility and exact octant-summary aggregation.
# Jupyter Notebookの新しい空のコードセルへ内容全体を貼り付けて実行してください。
# 単一セルで完結し、dense N x N行列は作りません。
...
def _select_local_kernel(
    ...
    for n_neighbors in neighbor_grid:
        ...
        for multiplier in multiplier_grid:
            ...
            if (
                np.asarray(validation_truth).ndim == 2
                and np.asarray(validation_truth).shape[1] == 1
                and np.asarray(prediction).ndim == 2
                and np.asarray(prediction).shape[1] == 1
            ):
                score_nrmse = _scalar_nrmse(
                    np.asarray(validation_truth)[:, 0],
                    np.asarray(prediction)[:, 0],
                    weights=validation_metric_weights,
                )
                metrics = {"normalized_rmse": float(score_nrmse)}
            else:
                metrics = velocity_metrics(
                    validation_truth,
                    prediction,
...
def _save_radial_mock3_figure(fig, stem):
    if SAVE_PDF:
...
def _select_scalar_local_kernel(
    validation_distances,
    validation_indices,
    train_values,
    train_source_weights,
    ...
    """Select a scalar local-kernel model using weighted radial NRMSE."""
    train_values = np.asarray(train_values, dtype=np.float64).reshape(-1, 1)
    validation_truth = np.asarray(validation_truth, dtype=np.float64).reshape(-1)
    best = None
    records = []
    for n_neighbors in neighbor_grid:
        bandwidth_neighbor = min(
            int(n_neighbors),
            max(1, int(round(float(n_neighbors) * float(bandwidth_fraction)))),
        )
        for multiplier in multiplier_grid:
            prediction = _local_kernel_from_neighbors(
                validation_distances,
            ...
            records.append(record)
            candidate = (float(score), int(n_neighbors), float(multiplier))
            if best is None or candidate < best[0]:
                best = (candidate, record)
    if best is None:
        raise RuntimeError("No scalar local-kernel model was evaluated.")
    record = best[1]
    return (
        int(record["n_neighbors"]),
        int(record["bandwidth_neighbor"]),
        float(record["bandwidth_multiplier"]),
...
def _build_or_load_observed_query_gradients(
    ...
    signature = _gradient_cache_signature(
        MOCK3_FILE, query_indices, geometry_source, constant_mode_index
    )
    if USE_GRADIENT_CACHE:
        gradient_cache_candidates = [
            PRIOR_RADIAL_GRADIENT_CACHE_FILE,
            GRADIENT_CACHE_FILE,
        ]
        seen_cache_paths = set()
        for cache_path in gradient_cache_candidates:
            cache_key = str(cache_path.resolve()) if cache_path.exists() else str(cache_path)
            if cache_key in seen_cache_paths or not cache_path.is_file():
                continue
            ...
            try:
                with np.load(cache_path, allow_pickle=False) as data:
                    if str(data["signature"].item()) != signature:
                        continue
                    print(f"[Gradient cache] reuse: {cache_path}")
                    return {
                ...
                warnings.warn(f"gradient cache could not be loaded ({cache_path}): {exc}")
...
PRIOR_MOCK3_DIR = ROOT_DIR / "mock3_octant_aware_loss_optimization_v4"
PRIOR_CACHE_DIR = PRIOR_MOCK3_DIR / "cache"
PRIOR_GEOMETRY_FILE = PRIOR_CACHE_DIR / "unweighted_alpha0_geometry_m512.npz"
PRIOR_TRUTH_FILE = PRIOR_CACHE_DIR / "independent_reference_truth_v4.npz"

# Reuse the already validated v1 observed/query gradient cache when its signature matches.
PRIOR_RADIAL_MOCK3_DIR = ROOT_DIR / "mock3_radial_potential_octant_selection_v1"
PRIOR_RADIAL_GRADIENT_CACHE_FILE = (
    PRIOR_RADIAL_MOCK3_DIR / "cache" / "potential_gradient_basis_observed_query_m512.npz"
)

OUTPUT_DIR = ROOT_DIR / "mock3_radial_potential_octant_selection_v2"
...
# Radial Mock-2 established unweighted loss as the primary family. The two weighted families below
# are pre-registered sensitivities and are not allowed to displace the primary family merely by
# winning the observed radial validation metric.
LOSS_FAMILIES = [
    {
        "name": "unweighted",
        "label": "Unweighted primary",
        "mode": "none",
        "gamma": 0.0,
        "cap": 1.0,
    },
    {
        ...
        "label": "Global-radial sensitivity",
        "mode": "global",
        "gamma": 0.75,
        "cap": 20.0,
    },
    {
        ...
        "label": "Octant-aware sensitivity",
        "mode": "region",
        "gamma": 1.0,
        "cap": 20.0,
    },
...
OUTPUT_PREFIX = "mock3_radial_potential_octant_selection"

SMOKE_TEST = os.environ.get("RADIAL_MOCK3_SMOKE_TEST", "0") == "1"
if SMOKE_TEST:
...
# --------------------------------------------------------------------------------------------------
# 2. Mock-3-specific helpers
# --------------------------------------------------------------------------------------------------
def _make_octant_stratified_split(region_id, train_fraction, validation_fraction, seed):
    ...
    for region in np.unique(region_id):
        ...
        n_train = int(round(train_fraction * local.size))
        n_validation = int(round(validation_fraction * local.size))
...
def _append_vector_evaluation(
    ...
    if np.sum(mask) < 1:
        return
    local_weights = None if weights is None else np.asarray(weights)[mask]
    metric = _radial_vector_metrics(
        np.asarray(true)[mask],
        np.asarray(predicted)[mask],
        np.asarray(line_of_sight)[mask],
        weights=local_weights,
    )
    record = {"model": model, "subset": subset, **metric}
    if extra:
        record.update(extra)
    records.append(record)


def _append_scalar_evaluation(
    records,
    model,
    subset,
    true,
    ...
    if mask is None:
        mask = np.ones(np.asarray(true).shape[0], dtype=bool)
    mask = np.asarray(mask, dtype=bool)
    if np.sum(mask) < 1:
        return
    local_weights = None if weights is None else np.asarray(weights)[mask]
    record = {
        ...
        "n": int(np.sum(mask)),
        "nrmse_radial": _scalar_nrmse(
            np.asarray(true)[mask], np.asarray(predicted)[mask], weights=local_weights
        ),
    }
    if extra:
        record.update(extra)
...
def _quantile_bin_records(
    ...
    selected = coordinate[base_mask]
    if selected.size < n_bins * 5:
        return []
    edges = np.unique(np.quantile(selected, np.linspace(0.0, 1.0, n_bins + 1)))
    records = []
    for index in range(edges.size - 1):
        lower = edges[index]
        upper = edges[index + 1]
        local = base_mask & (coordinate >= lower)
        if index == edges.size - 2:
            local &= coordinate <= upper
        else:
            local &= coordinate < upper
        if np.sum(local) < 5:
            continue
        records.append(
            {
                "model": model,
                "bin": int(index),
                f"{coordinate_name}_min": float(lower),
                f"{coordinate_name}_max": float(upper),
                f"{coordinate_name}_median": float(np.median(coordinate[local])),
                **_radial_vector_metrics(
                    true[local], predicted[local], line_of_sight[local]
                ),
            }
        )
    return records
...
def _family_probability(family, probability_global, probability_region):
    if family["mode"] == "none":
        return np.ones_like(probability_region)
    if family["mode"] == "global":
        return probability_global
    if family["mode"] == "region":
        return probability_region
    raise ValueError(f"unknown loss mode: {family['mode']}")

# --------------------------------------------------------------------------------------------------
# 3. Load catalogs, independent truth, octant-stratified split, and support masks
# --------------------------------------------------------------------------------------------------

start_time = time.perf_counter()
print("=" * 126)
print("Radial Mock-3: potential-flow reconstruction with direction-dependent selection")
print("=" * 126)
...
excluded_ids = np.union1d(mock2["ids"], survey["ids"])
...
query_region = (
    (centered_query[:, 0] >= 0.0).astype(np.int64)
    + 2 * (centered_query[:, 1] >= 0.0).astype(np.int64)
    + 4 * (centered_query[:, 2] >= 0.0).astype(np.int64)
)
...
query_m_lim_region = survey["mlim_per_region"][query_region]
probability_region_query = _selection_probability_from_mlim(
    query_dist, query_m_lim_region, survey["h"], lf_model
)
...
truth_signature = _stable_signature(
    {
        ...
        "mock3": _file_signature(MOCK3_FILE),
        "split_seed": int(COMPLETE_SPLIT_SEED),
...
split = _make_octant_stratified_split(
    survey["region_id"], TRAIN_FRACTION, VALIDATION_FRACTION, LABEL_SPLIT_SEED
)
...
line_of_sight_observed = centered_survey / np.linalg.norm(centered_survey, axis=1)[:, None]
line_of_sight_query = centered_query / np.linalg.norm(centered_query, axis=1)[:, None]
radial_truth_observed = np.einsum("ij,ij->i", line_of_sight_observed, truth_survey)
...
primary_metric_weights, primary_metric_info = _tempered_inverse_selection_weights(
    probability_region_survey, PRIMARY_METRIC_GAMMA, PRIMARY_METRIC_CAP
)
global_metric_weights, global_metric_info = _tempered_inverse_selection_weights(
    probability_global_survey, GLOBAL_METRIC_GAMMA, GLOBAL_METRIC_CAP
)
...
for region in range(8):
    for subset_name, indices in split.items():
        split_records.append(
            {
                "region_id": region,
                "m_lim": float(survey["mlim_per_region"][region]),
                "subset": subset_name,
                "n": int(np.sum(survey["region_id"][indices] == region)),
            }
        )
...
print(
    "Global-parent metric  : "
    f"gamma={GLOBAL_METRIC_GAMMA:g}, cap={GLOBAL_METRIC_CAP:g}, "
    f"ESS={global_metric_info['effective_sample_size']:.1f}"
)
display(split_df.pivot(index="region_id", columns="subset", values="n"))

# --------------------------------------------------------------------------------------------------
# 4. Unweighted diffusion geometry, query extension, and support definitions
# --------------------------------------------------------------------------------------------------
...
geometry["generator_eigenvalues"] = geometry["generator_eigenvalues"][:MAX_DIFFUSION_MODES]
...
query_extension = _nystrom_extend(centered_query, centered_survey, base_kernel, geometry)
...
sphere_boundary_safe = query_dist <= (SPHERE_RADIUS - REFERENCE_BANDWIDTH)

region_radial_limit = {
    region: float(
        np.quantile(survey["dist"][survey["region_id"] == region], 0.995)
    )
    for region in range(8)
}
region_radial_support = np.array(
    [
        query_dist[i] <= region_radial_limit[int(query_region[i])]
        for i in range(query_global.size)
    ],
    dtype=bool,
)
common_radial_limit = min(region_radial_limit.values())
common_radial_support = query_dist <= common_radial_limit

shallowest_m_lim = float(np.min(survey["mlim_per_region"]))
probability_shallowest_query = _selection_probability_from_mlim(
    query_dist,
    np.full(query_dist.shape, shallowest_m_lim),
    survey["h"],
    lf_model,
)

region_extended_support = (
    (probability_region_query >= SELECTION_SUPPORT_MIN)
    & neighbor_support
    & region_radial_support
)
region_strict_support = (
    (probability_region_query >= STRICT_SELECTION_SUPPORT_MIN)
    & neighbor_support
    & region_radial_support
)
region_primary_support = region_strict_support & sphere_boundary_safe
common_strict_support = (
    (probability_shallowest_query >= STRICT_SELECTION_SUPPORT_MIN)
    & neighbor_support
    & common_radial_support
    & sphere_boundary_safe
)

octant_boundary_distance_survey = np.min(np.abs(centered_survey), axis=1)
octant_boundary_distance_query = np.min(np.abs(centered_query), axis=1)
boundary_scale = float(np.median(base_kernel["rho"]))
near_octant_boundary = region_primary_support & (
    octant_boundary_distance_query <= boundary_scale
)
far_from_octant_boundary = region_primary_support & (
    octant_boundary_distance_query >= 2.0 * boundary_scale
)

query_subsets = {
    "query_all": np.ones(query_global.size, dtype=bool),
    "query_region_extended_support": region_extended_support,
    "query_region_primary_support": region_primary_support,
    "query_common_strict_support": common_strict_support,
    "query_region_primary_near_octant_boundary": near_octant_boundary,
    "query_region_primary_far_from_octant_boundary": far_from_octant_boundary,
    "query_high_completeness": region_primary_support & (probability_region_query >= 0.20),
    "query_medium_completeness": region_primary_support
    & (probability_region_query >= 0.05)
    & (probability_region_query < 0.20),
    "query_low_completeness": region_primary_support
    & (probability_region_query >= STRICT_SELECTION_SUPPORT_MIN)
    & (probability_region_query < 0.05),
}

support_df = pd.DataFrame(
    [{"subset": name, "n": int(np.sum(mask))} for name, mask in query_subsets.items()]
)
print("Query support")
display(support_df)

# --------------------------------------------------------------------------------------------------
# 5. Observed and out-of-sample metric-calibrated potential-gradient bases
# --------------------------------------------------------------------------------------------------
...
# Remove the best affine radial component from each diffusion-gradient mode.
...
column_rms = np.sqrt(np.mean(np.sum(np.square(observed_active), axis=2), axis=0))
...
potential_generator_values = np.concatenate([np.zeros(N_AFFINE_MODES), active_generator])
...
# --------------------------------------------------------------------------------------------------
# 6. Observed-to-query in-span closure
# --------------------------------------------------------------------------------------------------

print("-" * 126)
print("Test A: observed-to-complete-query potential in-span closure")
closure_rng = np.random.default_rng(20270501)
closure_records = []
reference_mask = region_primary_support if np.any(region_primary_support) else np.ones(query_global.size, dtype=bool)
reference_rms = float(
    np.sqrt(np.mean(np.sum(np.square(truth_query[reference_mask]), axis=1)))
)
for n_diffusion in CLOSURE_DIFFUSION_MODE_COUNTS:
    n_basis = N_AFFINE_MODES + int(n_diffusion)
    local_column_rms = np.sqrt(
        np.mean(np.sum(np.square(gradient_observed[:, :n_basis, :]), axis=2), axis=0)
    )
    coefficient_true = closure_rng.normal(size=n_basis) / np.maximum(local_column_rms, 1.0e-12)
    query_true = _potential_velocity(gradient_query, coefficient_true, n_basis)
    query_rms = float(np.sqrt(np.mean(np.sum(np.square(query_true[reference_mask]), axis=1))))
    if query_rms > 0.0:
        coefficient_true *= reference_rms / query_rms
    observed_true = _potential_velocity(gradient_observed, coefficient_true, n_basis)
    query_true = _potential_velocity(gradient_query, coefficient_true, n_basis)
    observed_radial = np.einsum("ij,ij->i", line_of_sight_observed, observed_true)
    design = radial_design_observed[fit_indices, :n_basis]
    ...
    query_pred = _potential_velocity(gradient_query, coefficient_estimated, n_basis)
    ...
    for support_name, support_mask in [
        ("region_primary", region_primary_support),
        ("common_strict", common_strict_support),
    ]:
        if np.sum(support_mask) < 1:
            continue
        metrics = _radial_vector_metrics(
            query_true[support_mask],
            query_pred[support_mask],
            line_of_sight_query[support_mask],
        )
        status = (
            "pass"
            if int(rank) == n_basis
        ...
        closure_records.append(
            {
                ...
                "n_query": int(np.sum(support_mask)),
...
# --------------------------------------------------------------------------------------------------
# 7. Pre-registered loss families and radial validation within each family
# --------------------------------------------------------------------------------------------------

print("-" * 126)
print("Test B: family-specific radial validation of potential hyperparameters")
weight_arrays = {}
weight_records = []
for family in LOSS_FAMILIES:
    probability = _family_probability(
        family, probability_global_survey, probability_region_survey
    )
    if family["mode"] == "none":
        weights = np.ones(survey["pos"].shape[0], dtype=np.float64)
        info = {
            "gamma": 0.0,
            "cap": 1.0,
            "probability_min": 1.0,
        ...
        weights, info = _tempered_inverse_selection_weights(
            probability, family["gamma"], family["cap"]
        )
    weight_arrays[family["name"]] = weights
    weight_records.append(
        {
            "loss_name": family["name"],
            "loss_label": family["label"],
            "mode": family["mode"],
            **info,
        }
    )
    ...
    for n_diffusion in POTENTIAL_DIFFUSION_MODE_COUNTS:
        ...
        for power in POTENTIAL_REGULARIZATION_POWERS:
            ...
            for strength in POTENTIAL_REGULARIZATION_STRENGTHS:
                ...
                metrics_octant = _radial_vector_metrics(
                    truth_survey[validation_indices],
                    vector_prediction,
                    line_of_sight_observed[validation_indices],
                    weights=primary_metric_weights[validation_indices],
                )
                metrics_global = _radial_vector_metrics(
                    truth_survey[validation_indices],
                    vector_prediction,
                    line_of_sight_observed[validation_indices],
                    weights=global_metric_weights[validation_indices],
                )
                validation_records.append(
                    {
                        "loss_name": loss_name,
                        "loss_label": family["label"],
                        "loss_mode": family["mode"],
                        "loss_gamma": float(family["gamma"]),
                        "loss_cap": float(family["cap"]),
                        "loss_ess_train": float(
                        ...
                        "regularization_strength": float(strength),
                        "penalty_power": float(power),
                        "octant_weighted_validation_nrmse_radial": _scalar_nrmse(
                            radial_truth_observed[validation_indices],
                            radial_prediction,
                            weights=primary_metric_weights[validation_indices],
                        ),
                        "global_weighted_validation_nrmse_radial": _scalar_nrmse(
                            radial_truth_observed[validation_indices],
                            radial_prediction,
                            weights=global_metric_weights[validation_indices],
                        ),
                        ...
                        "octant_weighted_validation_nrmse_tangential": metrics_octant[
                            "nrmse_tangential"
                        ],
                        "octant_weighted_validation_nrmse_3d": metrics_octant[
                            "nrmse_3d"
                        ],
                        "octant_weighted_validation_direction_error_median_deg": metrics_octant[
                            "direction_error_median_deg"
                        ],
                        "global_weighted_validation_nrmse_3d": metrics_global[
                            "nrmse_3d"
                        ],
                    }
                )
...
family_selected = {}
for family in LOSS_FAMILIES:
    family_selected[family["name"]] = _select_record(
        validation_df[validation_df["loss_name"] == family["name"]],
        [
...
operational_diagnostic = _select_record(
    validation_df,
    [
        "octant_weighted_validation_nrmse_radial",
        "global_weighted_validation_nrmse_radial",
...
selector_records = [
    {
        "selector": "primary_unweighted_family",
        "selection_role": "primary",
        **family_selected["unweighted"],
    },
    {
        "selector": "global_weighted_sensitivity",
        "selection_role": "sensitivity",
        **family_selected["global_g0p75_cap20"],
    },
    {
        "selector": "octant_weighted_sensitivity",
        "selection_role": "sensitivity",
        **family_selected["octant_g1p00_cap20"],
    },
    {
        "selector": "all_family_radial_selector_diagnostic",
        "selection_role": "diagnostic_only",
        **operational_diagnostic,
    },
    {
        "selector": "hidden_3d_validation_diagnostic",
        "selection_role": "diagnostic_only",
        **hidden_3d,
    },
]
...
# --------------------------------------------------------------------------------------------------
# 8. Final potential models, matched loss comparisons, and oracle ceilings
# --------------------------------------------------------------------------------------------------

selected_models = {}
model_specs = {
    "potential_unweighted_primary": family_selected["unweighted"],
    "potential_global_sensitivity": family_selected["global_g0p75_cap20"],
    "potential_octant_sensitivity": family_selected["octant_g1p00_cap20"],
...
# Matched on the primary unweighted potential structure isolates the weighting itself.
primary_structure = dict(family_selected["unweighted"])
for model_name, loss_name in [
    ("potential_matched_global_on_primary", "global_g0p75_cap20"),
    ("potential_matched_octant_on_primary", "octant_g1p00_cap20"),
]:
    spec = dict(primary_structure)
    family = next(item for item in LOSS_FAMILIES if item["name"] == loss_name)
    spec.update(
        {
            "loss_name": loss_name,
            "loss_label": family["label"],
            "loss_mode": family["mode"],
            "loss_gamma": family["gamma"],
            "loss_cap": family["cap"],
        }
    )
    selected_models[model_name] = _fit_final_radial_potential(
...
potential_oracle_n = int(potential_oracle_best["n_basis"])
selected_models["potential_3d_oracle"] = {
    "coefficients": potential_oracle_coefficients,
    "observed_prediction": _potential_velocity(
        gradient_observed, potential_oracle_coefficients, potential_oracle_n
    ),
    "query_prediction": _potential_velocity(
        gradient_query, potential_oracle_coefficients, potential_oracle_n
    ),
    "n_basis": potential_oracle_n,
    "regularization_strength": float(
        potential_oracle_best["regularization_strength"]
    ),
    "penalty_power": float(potential_oracle_best["penalty_power"]),
}
...
# Unweighted radial bulk-flow baseline.
bulk_design = line_of_sight_observed[fit_indices]
bulk_coefficient, _, _, _ = linalg.lstsq(
    bulk_design,
    radial_truth_observed[fit_indices],
    cond=None,
    lapack_driver="gelsd",
    check_finite=False,
)
selected_models["radial_bulk_flow"] = {
    "coefficients": bulk_coefficient,
    "observed_prediction": np.repeat(bulk_coefficient[None, :], survey["pos"].shape[0], axis=0),
    "query_prediction": np.repeat(bulk_coefficient[None, :], query_global.size, axis=0),
}
...
# --------------------------------------------------------------------------------------------------
# 9. Scalar local-radial baselines within the same three loss families
# --------------------------------------------------------------------------------------------------

validation_neighbor_distances, validation_neighbor_indices = _query_neighbors(
    centered_survey[validation_indices],
    centered_survey[train_indices],
    max(LOCAL_KERNEL_NEIGHBORS),
)
test_neighbor_distances, test_neighbor_indices = _query_neighbors(
    centered_survey[test_indices],
    centered_survey[fit_indices],
    max(LOCAL_KERNEL_NEIGHBORS),
)
query_neighbor_distances, query_neighbor_indices = _query_neighbors(
    centered_query,
    centered_survey[fit_indices],
    max(LOCAL_KERNEL_NEIGHBORS),
)
...
for family in LOSS_FAMILIES:
    loss_name = family["name"]
    weights = weight_arrays[loss_name]
    best = None
    for neighbors in LOCAL_KERNEL_NEIGHBORS:
        ...
        for multiplier in LOCAL_KERNEL_MULTIPLIERS:
            ...
            record = {
                ...
                "bandwidth_neighbor": int(bandwidth_neighbor),
                "bandwidth_multiplier": float(multiplier),
                "octant_weighted_validation_nrmse_radial": float(score),
            }
            local_validation_records.append(record)
            candidate = (float(score), int(neighbors), float(multiplier))
            if best is None or candidate < best[0]:
                best = (candidate, record)
    best_record = best[1]
    observed_test_prediction = _local_kernel_from_neighbors(
        test_neighbor_distances,
    ...
    local_predictions[f"local_radial_{loss_name}"] = {
        "observed_test_prediction": observed_test_prediction,
        "query_prediction": query_prediction,
        "n_neighbors": int(best_record["n_neighbors"]),
        "bandwidth_neighbor": int(best_record["bandwidth_neighbor"]),
        "bandwidth_multiplier": float(best_record["bandwidth_multiplier"]),
    }
local_validation_df = pd.DataFrame(local_validation_records)

# --------------------------------------------------------------------------------------------------
# 10. Observed, query, octant, support, boundary, completeness, and radial evaluation
# --------------------------------------------------------------------------------------------------
...
prediction_payload = {
    ...
    "probability_region_survey": probability_region_survey,
    "probability_global_survey": probability_global_survey,
    "probability_region_query": probability_region_query,
    "probability_global_query": probability_global_query,
    "region_primary_support": region_primary_support,
    "common_strict_support": common_strict_support,
    "octant_boundary_distance_query": octant_boundary_distance_query,
}

for model_name, model in selected_models.items():
    observed_prediction = model["observed_prediction"]
    query_prediction = model["query_prediction"]
    prediction_payload[f"{model_name}_observed_prediction"] = observed_prediction
    prediction_payload[f"{model_name}_query_prediction"] = query_prediction

    _append_vector_evaluation(
        evaluation_records,
        model_name,
    ...
    for region in range(8):
        local = survey["region_id"][test_indices] == region
        _append_vector_evaluation(
            evaluation_records,
            model_name,
            f"observed_test_octant_{region}",
            truth_survey[test_indices],
            observed_prediction[test_indices],
            line_of_sight_observed[test_indices],
            mask=local,
    ...
    for region in range(8):
        common_mask = common_strict_support & (query_region == region)
        region_mask = region_primary_support & (query_region == region)
        _append_vector_evaluation(
            evaluation_records,
            model_name,
            f"query_common_octant_{region}",
            truth_query,
            query_prediction,
            line_of_sight_query,
            mask=common_mask,
        ...
        _append_vector_evaluation(
            evaluation_records,
            model_name,
            f"query_region_specific_octant_{region}",
            truth_query,
            query_prediction,
            line_of_sight_query,
            mask=region_mask,
    ...
    radial_records.extend(
        _quantile_bin_records(
            model_name,
            truth_query,
            query_prediction,
            line_of_sight_query,
    ...
    for subset_name in [
        ...
        mask = query_subsets[subset_name]
        if np.sum(mask) < 5:
            continue
        completeness_records.append(
            {
                ...
                "selection_probability_median": float(
                    np.median(probability_region_query[mask])
                ),
                **_radial_vector_metrics(
                    truth_query[mask],
...
for model_name, model in local_predictions.items():
    ...
    _append_scalar_evaluation(
        ...
        radial_truth_query,
        model["query_prediction"],
        mask=region_primary_support,
    )
    _append_scalar_evaluation(
        local_evaluation_records,
        model_name,
        ...
        radial_truth_query,
        model["query_prediction"],
        mask=common_strict_support,
    )
local_evaluation_df = pd.DataFrame(local_evaluation_records)

# Octant dispersion summaries.
# Only exact octant rows with a finite region_id are aggregated. This excludes
# the aggregate row ``observed_test_octant_parent_weighted``, which shares the
# same textual prefix but is not a ninth octant.
def _build_exact_octant_summary(evaluation_table):
    summary_records = []
    for subset_prefix in [
        "observed_test_octant_",
        "query_common_octant_",
        "query_region_specific_octant_",
    ]:
        selected = evaluation_table[
            evaluation_table["subset"].astype(str).str.startswith(subset_prefix, na=False)
            & evaluation_table["region_id"].notna()
        ].copy()
        selected["region_id"] = pd.to_numeric(
            selected["region_id"], errors="coerce"
        )
        selected = selected[
            selected["region_id"].between(0, 7, inclusive="both")
        ].copy()
        selected["region_id"] = selected["region_id"].astype(int)

        for model_name, group in selected.groupby("model", sort=True):
            group = (
                group.sort_values("region_id")
                .drop_duplicates(subset=["region_id"], keep="last")
            )
            values = pd.to_numeric(
                group["nrmse_3d"], errors="coerce"
            ).to_numpy(dtype=np.float64)
            values = values[np.isfinite(values)]
            angles = pd.to_numeric(
                group["direction_error_median_deg"], errors="coerce"
            ).to_numpy(dtype=np.float64)
            angles = angles[np.isfinite(angles)]
            present_octants = sorted(group["region_id"].unique().tolist())
            missing_octants = sorted(set(range(8)) - set(present_octants))
            ...
            summary_records.append(
                {
                    "model": model_name,
                    "octant_subset": subset_prefix.rstrip("_"),
                    "n_octants": int(len(present_octants)),
                    "octant_ids": ",".join(str(value) for value in present_octants),
                    "missing_octant_ids": ",".join(
                        str(value) for value in missing_octants
                    ),
                    "nrmse_3d_mean": float(np.mean(values)),
                    "nrmse_3d_std": (
                        float(np.std(values, ddof=1)) if values.size > 1 else 0.0
                    ),
                    "nrmse_3d_worst": float(np.max(values)),
                    "nrmse_3d_best": float(np.min(values)),
                    "nrmse_3d_range": float(np.ptp(values)),
                    "direction_error_median_deg_mean": (
                        float(np.mean(angles)) if angles.size else math.nan
                    ),
                    "direction_error_median_deg_worst": (
                        float(np.max(angles)) if angles.size else math.nan
                    ),
                }
            )
...
octant_summary_df = _build_exact_octant_summary(evaluation_df)

observed_octant_counts = octant_summary_df.loc[
    octant_summary_df["octant_subset"] == "observed_test_octant",
    "n_octants",
]
if not observed_octant_counts.empty and not np.all(
    observed_octant_counts.to_numpy(dtype=int) == 8
):
    warnings.warn(
        "Corrected observed-test octant summary does not contain all eight "
        "octants for every model. Inspect the split and evaluation tables."
    )

# --------------------------------------------------------------------------------------------------
# 11. Linear-algebra diagnostics and paired bootstrap comparisons
# --------------------------------------------------------------------------------------------------

linear_algebra_records = []
singular_records = []
for family in LOSS_FAMILIES:
    specification = family_selected[family["name"]]
    n_basis = int(specification["n_basis"])
    design = radial_design_observed[fit_indices, :n_basis]
    weights = weight_arrays[family["name"]][fit_indices]
    gram, _ = _weighted_crossproducts(
        design, radial_truth_observed[fit_indices, None], weights
    )
    spectrum = _gram_spectrum(gram, fit_indices.size)
    regularization = _regularization_diagonal(
        potential_generator_values, n_basis, specification["penalty_power"]
    )
    df_eff = _effective_degrees_of_freedom(
        gram, regularization, specification["regularization_strength"]
    )
    linear_algebra_records.append(
        {
            "loss_name": family["name"],
            "loss_label": family["label"],
            "n_parameters": n_basis,
            ...
            "effective_degrees_of_freedom": df_eff,
            "effective_degrees_of_freedom_fraction": df_eff / n_basis,
            "regularization_strength": float(
    ...
    for index, value in enumerate(spectrum["singular_values"]):
...
singular_values_df = pd.DataFrame(singular_records)

comparison_pairs = [
    ("potential_global_sensitivity", "potential_unweighted_primary"),
    ("potential_octant_sensitivity", "potential_unweighted_primary"),
    ("potential_octant_sensitivity", "potential_global_sensitivity"),
    ("potential_matched_global_on_primary", "potential_unweighted_primary"),
    ("potential_matched_octant_on_primary", "potential_unweighted_primary"),
]
bootstrap_records = []
for pair_index, (model_a, model_b) in enumerate(comparison_pairs):
    for subset_name, true, los, mask, weights, prediction_key in [
        (
            "observed_test_octant_parent_weighted",
            truth_survey[test_indices],
            line_of_sight_observed[test_indices],
        ...
        if np.sum(mask) < 5:
            continue
        pred_a = selected_models[model_a][prediction_key]
        pred_b = selected_models[model_b][prediction_key]
        if subset_name.startswith("observed"):
            pred_a = pred_a[test_indices]
            pred_b = pred_b[test_indices]
        result = _paired_bootstrap_difference(
            ...
            BOOTSTRAP_SEED + 10 * pair_index + len(bootstrap_records),
        )
...
model_order = [
    "potential_unweighted_primary",
    "potential_global_sensitivity",
    "potential_octant_sensitivity",
...
labels = {
    "potential_unweighted_primary": "Potential unweighted primary",
    "potential_global_sensitivity": "Potential global sensitivity",
    "potential_octant_sensitivity": "Potential octant sensitivity",
...
observed_primary_df = evaluation_df[
    evaluation_df["subset"] == "observed_test_octant_parent_weighted"
].copy()
observed_primary_df["order"] = observed_primary_df["model"].map(
    {name: i for i, name in enumerate(model_order)}
)
observed_primary_df = observed_primary_df.sort_values("order")

query_region_df = evaluation_df[
    evaluation_df["subset"] == "query_region_primary_support"
].copy()
query_region_df["order"] = query_region_df["model"].map(
    {name: i for i, name in enumerate(model_order)}
)
query_region_df = query_region_df.sort_values("order")

query_common_df = evaluation_df[
    evaluation_df["subset"] == "query_common_strict_support"
].copy()
query_common_df["order"] = query_common_df["model"].map(
    {name: i for i, name in enumerate(model_order)}
)
query_common_df = query_common_df.sort_values("order")

print("Observed octant-parent-weighted test")
display(observed_primary_df.drop(columns="order"))
print("Complete-query region-specific primary support")
display(query_region_df.drop(columns="order"))
print("Complete-query common strict support")
display(query_common_df.drop(columns="order"))
print("Octant dispersion summary")
display(octant_summary_df)
...
# Selection curves for eight octants and global average.
radius_grid = np.linspace(max(0.5, np.min(query_dist)), SPHERE_RADIUS, 500)
fig, ax = plt.subplots(figsize=(8.6, 5.8))
for region, m_lim in enumerate(survey["mlim_per_region"]):
    curve = _selection_probability_from_mlim(
        radius_grid,
        np.full(radius_grid.shape, float(m_lim)),
        survey["h"],
        lf_model,
    )
    ax.plot(radius_grid, curve, label=f"octant {region}: $m_{{\\rm lim}}={m_lim:.3f}$")
global_curve = _global_mock3_probability(
    radius_grid, survey["h"], lf_model, survey["mlim_per_region"]
)
ax.plot(radius_grid, global_curve, linewidth=2.2, color="black", label="global average")
ax.axhline(STRICT_SELECTION_SUPPORT_MIN, linestyle="--", linewidth=1.0)
ax.axhline(SELECTION_SUPPORT_MIN, linestyle=":", linewidth=1.0)
ax.set_yscale("log")
ax.set_ylim(5.0e-5, 1.1)
ax.set_xlabel(r"Radius [$h^{-1}\,\mathrm{Mpc}$]")
ax.set_ylabel("Schechter selection probability")
ax.legend(fontsize="small", ncol=2)
fig.tight_layout()
_save_radial_mock3_figure(fig, "selection_curves")


def _plot_model_comparison(frame, ylabel, stem):
    selected = frame[frame["model"].isin(model_order)].copy()
    selected["order"] = selected["model"].map({name: i for i, name in enumerate(model_order)})
    selected = selected.sort_values("order")
    x = np.arange(selected.shape[0])
    width = 0.24
    fig, ax = plt.subplots(figsize=(12.5, 5.4))
    ax.bar(x - width, selected["nrmse_radial"], width, label="radial")
    ax.bar(x, selected["nrmse_tangential"], width, label="tangential")
    ax.bar(x + width, selected["nrmse_3d"], width, label="3D")
    ax.axhline(1.0, linestyle="--", linewidth=1.2, color="0.35", label="zero-vector baseline")
    ax.set_xticks(x)
    ax.set_xticklabels([labels[name] for name in selected["model"]], rotation=23, ha="right")
    ax.set_ylabel(ylabel)
    ax.legend(frameon=True, ncol=2)
    fig.tight_layout()
    _save_radial_mock3_figure(fig, stem)

_plot_model_comparison(
    observed_primary_df,
    "Octant-parent-weighted observed-test NRMSE",
    "observed_octant_parent_model_comparison",
)
_plot_model_comparison(
    query_region_df,
    "Region-specific complete-query NRMSE",
    "query_region_model_comparison",
)
_plot_model_comparison(
    query_common_df,
    "Common-support complete-query NRMSE",
    "query_common_model_comparison",
)

# Per-octant 3D NRMSE for region-specific and common support.
central_models = [
    "potential_unweighted_primary",
    "potential_global_sensitivity",
    "potential_octant_sensitivity",
]
for subset_prefix, stem, ylabel in [
    (
        "query_region_specific_octant_",
        "query_region_per_octant_3d_nrmse",
        "Region-specific query 3D NRMSE",
    ),
    (
        "query_common_octant_",
        "query_common_per_octant_3d_nrmse",
        "Common-support query 3D NRMSE",
    ),
]:
    fig, ax = plt.subplots(figsize=(8.8, 5.3))
    for model_name in central_models:
        selected = evaluation_df[
            (evaluation_df["model"] == model_name)
            & evaluation_df["subset"].str.startswith(subset_prefix)
        ].sort_values("region_id")
        ax.plot(selected["region_id"], selected["nrmse_3d"], marker="o", label=labels[model_name])
...
# Near/far octant-boundary comparison.
boundary_subset_order = [
    "query_region_primary_near_octant_boundary",
    "query_region_primary_far_from_octant_boundary",
]
fig, ax = plt.subplots(figsize=(8.4, 5.2))
positions = np.arange(len(central_models))
width = 0.36
for offset, (subset_name, label) in enumerate(
    [
        (boundary_subset_order[0], "near octant boundary"),
        (boundary_subset_order[1], "far from octant boundary"),
    ]
):
    selected = (
        evaluation_df[
            (evaluation_df["subset"] == subset_name)
            & evaluation_df["model"].isin(central_models)
        ]
        .set_index("model")
        .reindex(central_models)
    )
    ax.bar(positions + (offset - 0.5) * width, selected["nrmse_3d"], width=width, label=label)
ax.set_xticks(positions)
ax.set_xticklabels([labels[name] for name in central_models], rotation=20, ha="right")
ax.set_ylabel("Region-primary query 3D NRMSE")
ax.legend(frameon=True)
fig.tight_layout()
_save_radial_mock3_figure(fig, "octant_boundary_near_far_3d_nrmse")

# Boundary-distance and radius dependence.
for frame, coordinate, xlabel, stem in [
    (
        boundary_df,
        "boundary_distance_median",
        r"Distance from nearest octant boundary [$h^{-1}\,\mathrm{Mpc}$]",
        "query_3d_nrmse_by_boundary_distance",
    ),
    (
        radial_df,
        "radius_median",
        r"Radius [$h^{-1}\,\mathrm{Mpc}$]",
        "query_3d_nrmse_by_radius",
    ),
]:
    fig, ax = plt.subplots(figsize=(8.2, 5.2))
    for model_name in central_models:
        selected = frame[frame["model"] == model_name].sort_values(coordinate)
        ax.plot(selected[coordinate], selected["nrmse_3d"], marker="o", label=labels[model_name])
    ax.set_xlabel(xlabel)
    ax.set_ylabel("Region-primary complete-query 3D NRMSE")
    ax.legend(frameon=True)
    fig.tight_layout()
    _save_radial_mock3_figure(fig, stem)
...
fig, ax = plt.subplots(figsize=(8.2, 5.2))
for model_name in central_models:
    selected = completeness_df[completeness_df["model"] == model_name].sort_values(
...
ax.set_xscale("log")
ax.set_xlabel("Median region-specific selection probability")
ax.set_ylabel("Region-primary complete-query 3D NRMSE")
ax.legend(frameon=True)
fig.tight_layout()
_save_radial_mock3_figure(fig, "query_3d_nrmse_by_completeness")

# Direction-error CDF on region-specific primary support.
fig, ax = plt.subplots(figsize=(8.2, 5.4))
for model_name in central_models + ["cartesian_3d_oracle"]:
    prediction = selected_models[model_name]["query_prediction"]
    angles = _direction_errors(
        truth_query[region_primary_support], prediction[region_primary_support]
    )
...
ax.legend(frameon=True)
fig.tight_layout()
_save_radial_mock3_figure(fig, "query_region_direction_error_cdf")

# Family-specific validation summary.
family_best_frame = pd.DataFrame(
    [family_selected[family["name"]] for family in LOSS_FAMILIES]
)
fig, ax = plt.subplots(figsize=(7.8, 4.8))
positions = np.arange(family_best_frame.shape[0])
ax.scatter(
    positions,
    family_best_frame["octant_weighted_validation_nrmse_radial"],
    s=75,
)
ax.set_xticks(positions)
ax.set_xticklabels(family_best_frame["loss_label"], rotation=18, ha="right")
ax.set_ylabel("Octant-parent-weighted radial validation NRMSE")
fig.tight_layout()
_save_radial_mock3_figure(fig, "family_validation_summary")

# Primary unweighted singular spectrum.
primary_singular = singular_values_df[singular_values_df["loss_name"] == "unweighted"]
primary_linear = linear_algebra_df[linear_algebra_df["loss_name"] == "unweighted"].iloc[0]
fig, ax = plt.subplots(figsize=(7.6, 5.2))
ax.plot(primary_singular["singular_value_index"], primary_singular["normalized_singular_value"])
ax.set_yscale("log")
...
ax.text(
    0.04,
    0.06,
    f"rank = {int(primary_linear.numerical_rank)}/{int(primary_linear.n_parameters)}\n"
    f"condition number = {primary_linear.condition_number:.1f}\n"
    f"stable rank = {primary_linear.stable_rank:.1f}\n"
    f"entropy effective rank = {primary_linear.entropy_effective_rank:.1f}",
    transform=ax.transAxes,
    bbox=dict(boxstyle="round", facecolor="white", alpha=0.9),
)
fig.tight_layout()
_save_radial_mock3_figure(fig, "primary_design_singular_spectrum")

# Out-of-sample closure on region-primary support.
fig, ax = plt.subplots(figsize=(7.6, 5.2))
selected_closure = closure_df[closure_df["support"] == "region_primary"]
ax.plot(selected_closure["n_basis"], selected_closure["nrmse_3d"], marker="o")
...
ax.set_ylabel("Region-primary query in-span 3D NRMSE")
ax.legend(frameon=True)
fig.tight_layout()
_save_radial_mock3_figure(fig, "out_of_sample_in_span_closure")
...
weight_df.to_csv(OUTPUT_DIR / f"{OUTPUT_PREFIX}_loss_weight_diagnostics.csv", index=False)
split_df.to_csv(OUTPUT_DIR / f"{OUTPUT_PREFIX}_split.csv", index=False)
...
radial_df.to_csv(OUTPUT_DIR / f"{OUTPUT_PREFIX}_radial_bins.csv", index=False)
boundary_df.to_csv(OUTPUT_DIR / f"{OUTPUT_PREFIX}_boundary_bins.csv", index=False)
completeness_df.to_csv(
    OUTPUT_DIR / f"{OUTPUT_PREFIX}_completeness_subsets.csv", index=False
)
octant_summary_df.to_csv(
    OUTPUT_DIR / f"{OUTPUT_PREFIX}_octant_summary.csv", index=False
)
...
summary = {
    "experiment": "Radial Mock-3 potential-flow reconstruction with octant-dependent selection",
    "primary_model_family": "unweighted potential-flow loss",
    "sensitivity_families": [
        "global radial selection weighting",
        "octant-aware selection weighting",
    ],
    "family_selection_policy": (
        "loss family is pre-registered; only n_phi, lambda, and penalty power are selected "
        "within each family by octant-parent-weighted radial validation"
    ),
    ...
    "mlim_per_octant": survey["mlim_per_region"],
    "lf_model": {
        key: lf_model[key] for key in ["M_star", "alpha", "M_bright", "M_faint"]
    },
    "selectors": selector_df.to_dict(orient="records"),
    "potential_oracle_selection": potential_oracle_best,
    "cartesian_oracle_selection": cartesian_oracle["best"],
    "observed_primary_results": observed_primary_df.drop(columns="order").to_dict(orient="records"),
    "query_region_results": query_region_df.drop(columns="order").to_dict(orient="records"),
    "query_common_results": query_common_df.drop(columns="order").to_dict(orient="records"),
    "octant_summary": octant_summary_df.to_dict(orient="records"),
...
print("=" * 126)
print("Radial Mock-3 potential-flow octant-selection experiment completed")
print("=" * 126)
print(f"Elapsed time             : {(time.perf_counter() - start_time) / 60.0:.2f} min")
print("Primary and sensitivity selectors")
display(selector_df)
print("Observed octant-parent-weighted comparison")
display(observed_primary_df.drop(columns="order"))
print("Complete-query region-specific comparison")
display(query_region_df.drop(columns="order"))
print("Complete-query common-support comparison")
display(query_common_df.drop(columns="order"))
print("Local radial baselines")
display(local_evaluation_df)
print("Octant dispersion")
display(octant_summary_df)
